...
TEST(ProfilerTest, OverallStatistics) {
    ...
    ASSERT_EQ(stats->nsSumMiniCheckpoints, 0);
    ASSERT_EQ(stats->nsSumLostWork, 0);
    ASSERT_EQ(stats->numRecoveries, 0);
    ASSERT_EQ(stats->numMiniCheckpoints, 0);
    ASSERT_EQ(stats->nsSumRestoreModels, 0);
    ASSERT_EQ(stats->nsSumReloadSites, 0);
    ASSERT_EQ(stats->nsSumRecalculateAssignment, 0);
    ...
    ASSERT_EQ(stream->str(),
        ...
        "rank,processor,nsSumMiniCheckpoints,numMiniCheckpoints,numRecoveries,nsSumLostWork,nsSumRecalculateAssignment,nsSumReloadSites,nsSumRestoreModels\n0,Schneewittchen,1,2,3,4,5,6,7\n1,Zwerg,11,12,13,14,15,16,17\n"
    );
